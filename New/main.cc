...
int servermain() {
    ...
    while(1) {
        ...
        cout << "Connected!" << endl; 
	string fx=to_string(angle1);
	string fy=to_string(angle2);
        char buffer1[20];
        char buffer2[20];
        strcpy(buffer1,fx.c_str());
        strcpy(buffer2,fy.c_str());
	cout << buffer1;
	cout << buffer2;
        write(connfd, buffer1, 20);
        write(connfd, buffer2, 20);
        close(connfd);
...
int sensormain()
{
	...
	*/
	...
	//Kalman Filter
	angle1 = kalmanFilterX(AccXangle, rate_gyr_x);
	angle2 = kalmanFilterY(AccYangle, rate_gyr_y);
...
int main() {
	thread sensor(sensormain);
	thread server(servermain);
	sensor.join();
	server.join();
	return 0;
}
